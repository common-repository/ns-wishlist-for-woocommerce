<?php
add_action( 'woocommerce_before_add_to_cart_form', 'ns_ww4w_function_single');

function ns_ww4w_function_single(){
    global $post;
    ...
    $nsaddtowish = '<div id="nsw'.$post->ID.'" data-id="'.$post->ID.'" class="nswishlistclass nswSingleProduct'.$ns_color.'">
                        ...
                        '.get_option('ns_wishlist_font_awesome_pre', '<i class="fas fa-heart"></i>').' 
                            <span class="vNSalign">'.get_option('ns_whislist_label', 'Add to wishlist').'</span>
                        </div>
                    </div>';
    echo $nsaddtowish;
